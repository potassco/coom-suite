...
i.e. the `constraint` itself, one of the sub-expressions of a boolean constraint (`binary`, `unary`, `aggregate`), or parts of a table constraint (`allow`, `column`).
...
::: src/coomsuite/encodings/multi/new-aggregate.lp
    handler: asp
    options:
        encodings: true
