...
E.g. for `association` this function defines a condition `is_incremental_association`.

Third, `_get_incremental_prog_part` handles the conversion of a fact to its respective incremental program part.
The function determines the name of the program part based on the type of the fact (such as `association` or `binary`).
In simple cases (such as `unary` or `minimize`) a new type can just be added to one of the existing cases.
In more complex cases (such as `function` or `association`) a new case has to be added.
